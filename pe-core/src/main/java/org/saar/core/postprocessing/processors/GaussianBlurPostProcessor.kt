...
class GaussianBlurPostProcessor(contrast: Float) : PostProcessor,
    PostProcessorPrototypeWrapper(GaussianBlurPostProcessorPrototype(contrast)) {

}

private class GaussianBlurPostProcessorPrototype(private val contrast: Float) : PostProcessorPrototype {
    ...
    @UniformProperty
    private val blurLevelsUniform = UniformArray("u_blurLevels", blurLevels.size) { name, index ->
        object : FloatUniform() {
            override fun getName() = name

            override fun getUniformValue() = blurLevels[index]
        }
    }
    ...
    override fun fragmentShader(): Shader = Shader.createFragment(GlslVersion.V400,
        ShaderCode.define("LEVELS", blurLevels.size.toString()),
    ...
    fun onRender(context: PostProcessingContext, vertical: Boolean) {
        this.textureUniform.value = context.texture
        this.verticalBlurUniform.value = if (vertical) 1 else 0
    }
}